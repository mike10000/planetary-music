...
type BookArtistModalProps = {
  artistName: string;
  isOpen: boolean;
  onClose: () => void;
};

export default function BookArtistModal({
  artistName,
  isOpen,
  onClose,
}: BookArtistModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/book-artist", {
        method: "POST",
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="book-artist-title"
    >
      <div
      ...
      <div className="relative max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl sm:p-8">
        <div className="flex items-center justify-between">
          <h2 id="book-artist-title" className="text-xl font-bold text-[#1a2744]">
            Book {artistName}
          </h2>
          <button
            type="button"
        ...
        {status === "success" ? (
          <div className="mt-6 rounded-xl bg-green-50 p-6 text-center">
            <p className="text-lg font-medium text-green-800">
              Thank you! We&apos;ll get back to you soon about booking {artistName}.
            </p>
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            ...
            <div>
              <label htmlFor="book-email" className="block text-sm font-medium text-gray-700">
                Email *
              </label>
              <input
                type="email"
            ...
            <div>
              <label htmlFor="book-phone" className="block text-sm font-medium text-gray-700">
                Phone *
              </label>
              <input
                type="tel"
            ...
            <div>
              <label htmlFor="book-eventDate" className="block text-sm font-medium text-gray-700">
                Event Date (if known)
              </label>
              <input
                type="text"
            ...
            <div>
              <label htmlFor="book-venue" className="block text-sm font-medium text-gray-700">
                Venue / Location
              </label>
              <input
                type="text"
            ...
            <div>
              ...
              <textarea
                ...
                required
                rows={4}
                placeholder="Event type, expected guests, preferred style..."
                className="mt-1 block w-full rounded-lg border border-gray-300 px-4 py-2 shadow-sm focus:border-[#1a2744] focus:ring-[#1a2744]"
              />
            </div>
            {status === "error" && (
